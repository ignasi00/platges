...
from docopts.help_segmentation_train_platgesBCN import parse_args
...
# TODO: criterion wherw class 3 is either 1 or 2
...
LIST_PATH_TRAIN = "./data_lists/platgesbcn2021_fine.csv"
LIST_PATH_VAL = "./data_lists/platgesbcn2021_wide.csv"
...
def build_train_dataset(list_path, resize_height, resize_width, crop_height, crop_width, mean, std, scale_limit, shift_limit, rotate_limit):
    platgesBCN_dataset = PlatgesBCNSegmentationDataset(list_path)
    ...
    platgesBCN_seg_dataset = TransformDataset(platgesBCN_dataset, transforms, drop_extra_params=True)
    return platgesBCN_seg_dataset
...
def main(experiment_name, project_name, entity, list_path_train, list_path_val, outputs_root, models_path, resize_height, resize_width, crop_h, crop_w, scale_limit, shift_limit, rotate_limit, batch_size, learning_rate, num_epochs, layers, num_classes_pretrain, zoom_factor, backbone_output_stride, backbone_net, pretrained_back_path, pretrained_path, funnel_map, device=None, model_name=None, VERBOSE_BATCH=True, VERBOSE_END=True):
    ...
    ####################################### PREPROCESSING  #######################################
    mean, std = get_mean_and_std(PlatgesBCNSegmentationDataset(list_path_train), value_scale=255) # TODO: ¿concatenated train and val?

    platgesBCN_seg_train_dataset = build_train_dataset(list_path_train, resize_height, resize_width, crop_h, crop_w, mean, std, scale_limit, shift_limit, rotate_limit)
    platgesBCN_seg_val_dataset = build_val_dataset(list_path_val, resize_height, resize_width, crop_h, crop_w, mean, std)
    ...
    platgesBCN_seg_train_dataloader = DataLoader(platgesBCN_seg_train_dataset, batch_size=min(batch_size, MAX_BATCH_SIZE), collate_fn=collate_fn)
    platgesBCN_seg_val_dataloader = DataLoader(platgesBCN_seg_val_dataset, batch_size=min(batch_size, MAX_BATCH_SIZE), collate_fn=collate_fn)
    ...
    platgesBCN_seg_train_local_logger = LocalLogger(metric_funct_dict, len(platgesBCN_seg_train_dataset), prefix="Train")
    platgesBCN_seg_val_local_logger = LocalLogger(metric_funct_dict.copy(), len(platgesBCN_seg_val_dataset), prefix="Valid")
    ...
    for epoch in range(num_epochs): # TODO: load from epoch => offset where needed (by instance, on best_epoch)
        ...
        #last_train_epoch_log = accumulated_grad_train(model, criterion, optimizer, platgesBCN_seg_train_dataloader, platgesBCN_seg_train_local_logger, batch_size, device=device, drop_last=True, VERBOSE_BATCH=VERBOSE_BATCH, VERBOSE_END=VERBOSE_END)
        last_train_epoch_log = accumulated_grad_train(model, criterion, optimizer, platgesBCN_seg_train_dataloader, platgesBCN_seg_train_local_logger, batch_size, drop_last=True, VERBOSE_BATCH=VERBOSE_BATCH, VERBOSE_END=VERBOSE_END)
        wandb_logger.log(last_train_epoch_log, prefix="train_", step=epoch)
        platgesBCN_seg_train_local_logger.print_last_epoch_summary(mode='train')

        model.train()
        model.eval()

        with torch.no_grad():
            #last_val_epoch_log = vanilla_validate(model, criterion, platgesBCN_seg_val_dataloader, platgesBCN_seg_val_local_logger, device=device, VERBOSE_BATCH=VERBOSE_BATCH, VERBOSE_END=VERBOSE_END)
            last_val_epoch_log = vanilla_validate(model, criterion, platgesBCN_seg_val_dataloader, platgesBCN_seg_val_local_logger, VERBOSE_BATCH=VERBOSE_BATCH, VERBOSE_END=VERBOSE_END)
        ...
        platgesBCN_seg_val_local_logger.print_last_epoch_summary(mode='valid')
    ...
    ####################################### -------------- #######################################
    best_epoch = platgesBCN_seg_val_local_logger.best_epochs()[0]

    wandb_logger.update_model(f'epoch_{best_epoch}', ['best'])

    wandb_logger.summarize({'best_epoch' : best_epoch})
    wandb_logger.summarize(platgesBCN_seg_train_local_logger.get_one_epoch_log(best_epoch, prefix="train_"))
    wandb_logger.summarize(platgesBCN_seg_val_local_logger.get_one_epoch_log(best_epoch, prefix="valid_"))
